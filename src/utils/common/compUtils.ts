...
import { normalizeFileAccessHttpUrl } from './fileUrl';
//存放部门路径的数组
...
export const getFileAccessHttpUrl = (fileUrl, prefix = 'http') => {
  return normalizeFileAccessHttpUrl(fileUrl, baseApiUrl, prefix);
};
/**
 *  获取桌面端wps的文件服务访问路径
 * @param fileUrl 文件路径
...
export function getMultiDepartPathName(title,id) {
  ...
  if(id instanceof Array){
    ...
    postIds = String(id).split(",")
  }
  let postNames = "";
  postIds.forEach((postId)=>{
    const postName = getDepartPathName(title,postId,false);
    postName && (postNames += postName + ",");
  });
...
export function getDepartName(departNamePath) {
  if(departNamePath){
    const names = (departNamePath instanceof Array ? departNamePath : String(departNamePath).split(",")).filter(Boolean);
