...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      /**
       * Upload gateways return the stored object path in different fields across
       * deployments. A human-readable `message` is deliberately not a fallback:
       * persisting it as an avatar would create a broken image URL.
       */
      function getUploadedFilePath(response: Record<string, any>): string {
        if (response?.success === false) {
          throw new Error(response?.message || '头像上传失败');
        }
        const payload = response?.result ?? response?.data ?? response?.url;
        const structuredPath = typeof payload === 'string'
          ? payload
          : payload?.url ?? payload?.path ?? payload?.filePath;
        const messagePath = typeof response?.message === 'string' && (
          /^(https?:\/\/|\/)/i.test(response.message.trim())
          || response.message.includes('/')
        )
          ? response.message
          : '';
        const path = structuredPath || messagePath;

        if (typeof path !== 'string' || !path.trim()) {
          throw new Error(response?.message || '上传服务未返回图片地址');
        }
        return path.trim();
      }
        ...
        if (uploadApi && isFunction(uploadApi)) {
          ...
          try {
            ...
            emit('uploadSuccess', {
              ...
              data: getUploadedFilePath(result),
            });
            closeModal();
          } catch (error) {
            createMessage.error(error instanceof Error ? error.message : '头像上传失败，请重试');
          } finally {
