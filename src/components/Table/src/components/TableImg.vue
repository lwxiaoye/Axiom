<template>
  <div :class="prefixCls" class="flex items-center mx-auto" v-if="imgList && imgList.length" :style="getWrapStyle">
    ...
    <PreviewGroup v-else>
      <template v-for="(img, index) in imgList" :key="img">
        <Image :width="size" :style="{ marginLeft: index === 0 ? 0 : margin }" :src="getImageUrl(img)" />
      </template>
    </PreviewGroup>
  </div>
</template>
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      function getImageUrl(img: string) {
        return getFileAccessHttpUrl(`${props.srcPrefix || ''}${img || ''}`);
      }

      const { prefixCls } = useDesign('basic-table-img');
      return { prefixCls, getWrapStyle, getImageUrl };
    },
  });
</script>
<style lang="less">
