...
export type PortableRunSkinDevice = 'desktop' | 'tablet' | 'mobile';

export type PortableRunSkinAsset = {
  key: string;
  path: string;
...
export type PortableRunSkinDecoration = {
  asset: string;
  anchor:
    | 'page-top-left'
    | 'page-top-right'
...
export type PortableRunSkinLayout = {
  background: {
    asset?: string;
    fit: 'cover' | 'contain';
  ...
  content: { maxWidth: number; topGap: number };
  decorations: PortableRunSkinDecoration[];
};

export type PortableRunSkinManifest = {
  ...
  assets: PortableRunSkinAsset[];
  theme: {
    colors: Partial<Record<
      'page' | 'title' | 'body' | 'accent' | 'composer' | 'composerBorder' | 'userBubble' | 'userBubbleText',
      string
    >>;
    composer: { radius?: number; shadow?: 'none' | 'soft' | 'elevated' };
  };
  layouts: Record<PortableRunSkinDevice, PortableRunSkinLayout>;
};

/** 后端校验并投影的只读运行时字段，不写入工作流 JSON。 */
export type PortableRunSkinRecord = {
  id: string;
  scope: 'sub_agent';
  key: string;
  ...
  name: string;
  description: string;
  renderer: 'decorated-agent-run-v1' | string;
  contentHash: string;
  sourceType: string;
  status: string;
  ...
  deletable?: boolean;
  manifest: PortableRunSkinManifest | null;
};

/**
 * 独立运行页的外观预设配置。
 *
 * 数据层只持久化稳定的 preset 标识与少量安全文案；具体图片、组件和 CSS 均由前端预设注册表提供，
 * 避免把可执行样式或任意资源地址写进业务数据。
 */
export type RunPresentationConfig = {
  schemaVersion?: 1;
  preset?: string;
  copy?: {
    welcomeTitle?: string;
