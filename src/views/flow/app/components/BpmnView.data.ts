...
export const formSchema1: FormSchema[] = [
  ...
  },
  {
    ...
    field: 'auditType',
    required: ({ values }) => {
      return values.startUserSpecify != 'true';
    },
    component: 'Select',
    componentProps: {
      options: [
        ...
        // { value: 'startUserSet', label: '发起人指定' },
      ],
    },
  },
  {
  ...
  },
  {
    ...
    componentProps: {
      ...
      onOptionsChange: (options) => {
        console.log('返回的options:', options);
        options.forEach((item) => {
          if (item.roleCode == 'jzg' || item.roleCode == 'xs') {
            item.disabled = true;
          }
        });
        // 这里可以处理options，比如存储到变量中
      },
    },
    ifShow: ({ values }) => {
...
export const getDefaultXml = (processDefinitionKey: any, appName: any) => {
...
<bpmn:definitions xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" 
  ...
  <bpmn:process id="${processDefinitionKey}" name="${appName}" isExecutable="true" camunda:historyTimeToLive="36500">
