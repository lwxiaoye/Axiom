  ...
  <BasicModal v-bind="$attrs" @register="registerModal" destroyOnClose :title="title" :width="1500" :height="contentHeight" @ok="handleSubmit">
    <a-tabs height="100%" v-model:activeKey="activeKey" animated @change="handleChangeTabs" centered>
      <a-tab-pane tab="基本信息" key="basicInfo">
        <BasicForm @register="registerForm" ref="formRef" @change-fn="changeFn" :style="`height:${height}px;padding-top:12px`" name="AppInfoForm">
          <template #CropperImg="{ model, field }">
            <CropperAvatar
      ...
      <a-tab-pane tab="表单设计" key="formInfo" v-if="formType == 'flow'">
        <fc-designer
          ref="designerRef"
          :config="{
            formOptions: {
      ...
      <a-tab-pane tab="流程设计" key="flowInfo" v-if="formType == 'flow'">
        <BpmnView ref="bpmRef" :processDefinitionKey="props.processDefinitionKey" />
      </a-tab-pane>
    </a-tabs>
  </BasicModal>
</template>
...
<script lang="ts" setup>
  ...
  import { formSchema } from '../AppInfo.data';
  ...
  const bpmRef = ref<any>(null);
  const formData = ref<any>({});
  // Emits声明
  const emit = defineEmits(['register', 'success']);
  const isUpdate = ref<any>(true);
  const isDetail = ref<any>(false);
  // const refKeys = ref(['basicInfo', 'formInfo', 'flowInfo']);
  const activeKey = ref<any>('basicInfo');
  const editData = ref<any>(null);
  const height = ref<any>(0);
  const contentHeight = ref<any>(0);
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (unref(isUpdate)) {
      ...
      const appRoles = await queryAppRole({ appId: data.record.id });
      if (appRoles && appRoles.length > 0) {
        data.record.selectedRoles = appRoles;
      }
      const appDepts = await queryAppDept({ appId: data.record.id });
      if (appDepts && appDepts.length > 0) {
        data.record.selectedDeparts = appDepts;
      }
  ...
  // 获取选择的change的
  let formType = ref<any>('');
  function changeFn(e: any) {
    console.log(e._object.formModel);
    if (e._object.formModel.appType) {
      formType.value = e._object.formModel.appType;
    } else {
      formType.value = '';
    }
  }
  ...
  async function handleSubmit() {
    ...
    if (bpmRef.value) {
      bpmRef.value.getFile(async (xml: any) => {
