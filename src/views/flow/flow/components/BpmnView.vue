...
import { ElMessage } from 'element-plus';
  ...
  import { getBpmnDetails } from '../FlowInfo.api';
  ...
  const [registerForm1, { resetFields: resetFields1, setFieldsValue: setFieldsValue1, getFieldsValue: getFieldsValue1 }] = useForm({
    labelWidth: 100,
    schemas: formSchema1,
    showActionButtonGroup: false,
    baseColProps: { span: 24 },
  });
  const [registerForm2, { resetFields: resetFields2, setFieldsValue: setFieldsValue2, getFieldsValue: getFieldsValue2 }] = useForm({
    labelWidth: 105,
    schemas: formSchema2,
    showActionButtonGroup: false,
    baseColProps: { span: 24 },
  ...
  function clickUserTask(element: any) {
    //初始化用户任务面板默认值
    initUserTaskValue();
    //回显节点
  ...
  //初始化流程图
  function init1(processDefinitionKey: any) {
    //通过接口获取流程图
    getBpmnDetails(processDefinitionKey).then((res: any) => {
      if (res) {
        modeler.importXML(res);
      } else {
        modeler.importXML(getDefaultXml(props.processDefinitionKey));
      }
  ...
  onMounted(() => {
    ...
    init1(props.processDefinitionKey);
    bpmnFactory = modeler.get('bpmnFactory');
  });

  const changeFn = (schema) => {
