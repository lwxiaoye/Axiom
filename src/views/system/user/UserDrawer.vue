...
<script lang="ts" setup>
  ...
  import { saveOrUpdateUser, getUserRoles, getUserIdentityCard, getUserDepartList, getAllRolesListNoByTenant, getAllRolesList } from './user.api';
  ...
  //表单配置
  const [registerForm, { setProps, resetFields, resetSchema, setFieldsValue, validate, updateSchema }] = useForm({
    labelWidth: 90,
    schemas: cloneDeep(formSchema),
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    await resetSchema(cloneDeep(formSchema));
    await resetFields();
    departOptions.value = [];
   ...
   /*   if (data.record.relTenantIds && !Array.isArray(data.record.relTenantIds)) {
      ...
      try {
        data.record.idCard = await getUserIdentityCard({ id: data.record.id });
      } catch (error) {}
  ...
  async function handleSubmit() {
    try {
      ...
      // 租户关联改由专门的租户管理入口维护，用户抽屉不提交此字段。
      delete values.relTenantIds;
      ...
      await nextTick();
      //刷新列表
      emit('success', { isUpdate: isUpdateVal, isUpdateVal, values });
    } finally {
      setDrawerProps({ confirmLoading: false });
    }
  }
