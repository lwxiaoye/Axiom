...
| 项 | 内容 |
| --- | --- |
| 用途 | 本机开发时连接自己的 Java 业务后端和数据库 |
| 关系 | 日常起停以 [`本地热更新开发工作流.md`](本地热更新开发工作流.md) 为准。本文只补网络排障 |
| Java | 开发模式默认 `127.0.0.1:9090`。不要把 Vite 代理改到未接入的旧地址 |
...
浏览器 → http://127.0.0.1:3200
           ├─ /api、/upload     → 127.0.0.1:9090     自己的 Java 业务后端
           └─ /agent-api        → 127.0.0.1:8000     本机 Agent API 或 Docker 映射端口
                                      ├─ MySQL / PostgreSQL / Qdrant
                                      └─ 模型网关（可选）
```

页面 HTML 200 只说明 Vite 能返回页面。登录失败应先检查 Java 代理与鉴权响应；对话 503 可能来自 Runtime schema、Worker、鉴权回源或其他依赖。先读 `/health/ready` 的具体检查结果。

不要用 ping 判断内网服务是否可用，只认 TCP 与 HTTP 响应。

## 2. 本机 Java（`127.0.0.1:9090`）

本机必须能到达实际部署 Java 的主机。系统代理或 TUN 模式可能劫持 `127.0.0.1`，表现为 TCP 能通但 HTTP 空响应。

处理：

1. 把本机回环地址和局域网服务设为直连，或临时关闭 TUN 后再测。
2. 确认当前网络能访问自己的 Java / 数据库，而不是误连到其他环境。
