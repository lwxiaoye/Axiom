...
This catalog is a design reference, not an installation guarantee. Before choosing fonts, run `fc-list : family` (or the platform-equivalent font inventory) in the actual export environment. Use only exact installed family names unless the deck declares a reachable `customFonts` source and the chosen exporter is verified to load it.

For every generated deck, declare at least two named roles in `theme.textStyles`, such as `display`/`title` and `body`. Keep the deck to 2–3 families. Verify the rendered pages, and never claim font embedding unless the exported PPTX actually contains both `ppt/fonts/*.fntdata` parts and an `embeddedFontLst` relationship.
