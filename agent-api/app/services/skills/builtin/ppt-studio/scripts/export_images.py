...
def export_images_offline(
    source: Path,
    output: Path,
    force: bool = False,
) -> Dict[str, Any]:
    """Render via local WASM PPTX export + LibreOffice + Poppler.

    The production sandbox deliberately ships Node without npm or a browser daemon.
    LibreOffice and pdftoppm are already part of the artifact-review runtime, so this
    path provides deterministic offline page images without installing dependencies or
    weakening the final independent review performed by ``publish_ppt_artifact``.
    """
    ...
    if not libreoffice or not pdftoppm:
        raise ExportError("offline image export requires libreoffice and pdftoppm")

    payload = build_payload(manifest)
    image_cls, draw_cls, image_font = ensure_pillow()
    with temporary_directory(prefix="open-kimi-ppt-offline-images-") as temp_name:
        temp_dir = Path(temp_name)
        pptx = temp_dir / "deck.pptx"
        exporter = Path(__file__).resolve().with_name("export_pptx.py")
        result = run_command(
            [sys.executable, str(exporter), str(manifest), "--output", str(pptx)],
            timeout=300,
        )
        if result.returncode != 0 or not pptx.is_file():
            raise ExportError(f"offline PPTX export failed:\n{result.stdout[-4000:]}")

        result = run_command(
            [libreoffice, "--headless", "--convert-to", "pdf", "--outdir", str(temp_dir), str(pptx)],
            timeout=180,
        )
        pdfs = sorted(temp_dir.glob("*.pdf"))
        if result.returncode != 0 or not pdfs:
            raise ExportError(f"LibreOffice PDF conversion failed:\n{result.stdout[-4000:]}")
        rendered_prefix = temp_dir / "page"
        result = run_command(
            [pdftoppm, "-png", "-r", "120", str(pdfs[0]), str(rendered_prefix)],
            timeout=240,
        )
        images = sorted(temp_dir.glob("page-*.png"), key=page_sort_key)
        if result.returncode != 0 or not images:
            raise ExportError(f"pdftoppm rendering failed:\n{result.stdout[-4000:]}")
        ...
        for index, image in enumerate(images, start=1):
            target = pages_dir / f"{index:02d}.png"
            shutil.copy2(image, target)
            copied.append(target)
        overview = stitch_overview(
            copied, output / "overview.jpg", image_cls, draw_cls, image_font
        )
    ...
    mapping = [
        ...
        for index, path in enumerate(copied, start=1)
    ]
    return {
        "pages": len(copied),
        ...
        "output": str(output),
        "images": mapping,
        "exporter": "local-wasm-libreoffice-poppler",
    }
...
def export_images(
    ...
    manifest = find_manifest(source)
    if (shutil.which("libreoffice") or shutil.which("soffice")) and shutil.which("pdftoppm"):
        return export_images_offline(source, output, force)
    payload = build_payload(manifest)
    output = output.expanduser().resolve()
    if output.exists() and any(output.iterdir()) and not force:
        raise ExportError(
