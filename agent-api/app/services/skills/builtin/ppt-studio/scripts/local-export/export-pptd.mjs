...
import { normalizePptdRichTextForWasm } from './normalize-rich-text.mjs';
import { normalizePptdThemeStyleRefsForWasm } from './normalize-theme-styles.mjs';
...
async function loadProject(manifestPath, parseYaml) {
  ...
  for (const rel of manifest.pages) {
    ...
    pages.push({
      ...page,
      background: normalizeFill(page.background) ?? page.background,
      pagePath: rel.replace(/\\/g, '/'),
      elements: page.elements.map((element, index) => normalizeElement(element, index)),
    });
  }
...
function normalizeFill(fill) {
  ...
  if (fill.type) return fill;
  if (typeof fill.color === 'string') return { type: 'solid', color: fill.color };
  return fill;
}

function normalizeBorder(border) {
  if (!border || typeof border !== 'object') return border;
  if (border.style || border.width || border.color) {
    return {
...
function normalizeLegacyTextContent(content) {
  if (!Array.isArray(content)) return content;
  const runs = content.filter((item) => item && typeof item === 'object');
  if (!runs.length) return { text: '' };
  const first = runs[0];
  const allowed = [
    'style', 'color', 'fontSize', 'fontFamily', 'bold', 'italic',
    'backgroundColor', 'lineHeight', 'lineHeightPx', 'letterSpacing',
    'marginTop', 'textDirection', 'wrap', 'align', 'gradient', 'shadow',
  ];
  const normalized = { text: runs.map((run) => String(run.text ?? '')).join('\n') };
  for (const key of allowed) {
    if (first[key] !== undefined) normalized[key] = first[key];
  }
  return normalized;
}

function normalizeElement(el, index = 0) {
  // Compatibility bridge for the concise x/y/w/h dialect often produced by
  // non-Kimi models.  Canonical PPTD remains unchanged; the bridge only fills
  // the exact v2 element contract before calling the official WASM writer.
  if (!el?.elementType && typeof el?.type === 'string') {
    const kind = String(el.type).toLowerCase();
    const x = Number(el.x ?? Math.min(Number(el.x1 || 0), Number(el.x2 || 0)));
    const y = Number(el.y ?? Math.min(Number(el.y1 || 0), Number(el.y2 || 0)));
    const width = Number(el.w ?? Math.abs(Number(el.x2 || 0) - Number(el.x1 || 0)));
    const height = Number(el.h ?? Math.abs(Number(el.y2 || 0) - Number(el.y1 || 0)));
    const elementId = String(el.elementId || `legacy-${kind}-${index + 1}`);
    if (kind === 'text') {
      return {
        elementId,
        elementType: 'text',
        bounds: [x, y, width, height],
        content: normalizeLegacyTextContent(el.content),
    ...
    if (kind === 'rect' || kind === 'rectangle') {
      ...
      return {
        ...
        elementType: 'shape',
        bounds: [x, y, width, height],
        shapeName: radius > 0 ? 'roundRect' : 'rect',
        ...(radius > 0 ? { adjustments: [Math.min(50000, Math.max(1000, Math.round(radius * 1000)))] } : {}),
        ...(el.fill ? { fill: normalizeFill(el.fill) } : {}),
        ...(el.stroke ? { border: normalizeBorder(el.stroke) } : {}),
        ...(el.opacity !== undefined ? { opacity: el.opacity } : {}),
      };
    }
    if (kind === 'line') {
      const x1 = Number(el.x1 ?? x);
      const y1 = Number(el.y1 ?? y);
      const x2 = Number(el.x2 ?? (x + width));
      const y2 = Number(el.y2 ?? (y + height));
      const minX = Math.min(x1, x2);
      const minY = Math.min(y1, y2);
      const deltaX = Math.abs(x2 - x1);
      const deltaY = Math.abs(y2 - y1);
      const viewW = Math.max(1, deltaX);
      const viewH = Math.max(1, deltaY);
      const startX = deltaX === 0 ? 0 : (x1 <= x2 ? 0 : viewW);
      const startY = deltaY === 0 ? 0 : (y1 <= y2 ? 0 : viewH);
      const endX = deltaX === 0 ? 0 : (x1 <= x2 ? viewW : 0);
      const endY = deltaY === 0 ? 0 : (y1 <= y2 ? viewH : 0);
      return {
        elementId,
        elementType: 'line',
        bounds: [minX, minY, viewW, viewH],
        viewBox: [viewW, viewH],
        points: `${startX},${startY} ${endX},${endY}`,
        curve: 'sharp',
        border: normalizeBorder(el.stroke || el.border || {}),
      };
    }
    throw new Error(`Unsupported legacy PPTD element type: ${el.type}`);
  }
  ...
  if (el.elementType === 'shape') {
    return ensureViewBox({
      ...el,
      ...(el.fill ? { fill: normalizeFill(el.fill) } : {}),
      ...(el.border ? { border: normalizeBorder(el.border) } : {}),
...
Env: KIMI_COOKIE, KIMI_ORIGIN`);
  ...
  normalizePptdThemeStyleRefsForWasm(pptd);
  normalizePptdRichTextForWasm(pptd);
