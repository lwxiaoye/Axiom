...
SANDBOX_EXPORT_DIR = Path("/opt/open-kimi-ppt/scripts/local-export")
...
def resolve_local_wasm() -> Path:
    ...
    candidates = [
        ...
        SANDBOX_EXPORT_DIR / "pptd_wasm_bg.wasm",
    ]
    for candidate in candidates:
...
def resolve_local_export_mjs() -> Path:
    for candidate in (LOCAL_EXPORT_MJS, SANDBOX_EXPORT_DIR / "export-pptd.mjs"):
        if candidate.is_file():
            return candidate
    raise ExportError(
        f"local exporter missing: {LOCAL_EXPORT_MJS} and "
        f"{SANDBOX_EXPORT_DIR / 'export-pptd.mjs'}"
    )
...
def export_pptx_local(
    ...
    export_mjs = resolve_local_export_mjs()
