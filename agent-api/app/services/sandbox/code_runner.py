"""run_code 执行内核（ADR-047 主对话第三层「做」）。
...
- 交付物写 /workspace/files/（统一文件系统，写进去就是已保存）。
  /workspace/outputs 是 run_code 时代的产物目录，现成技能里仍到处硬编码它——`_mkdir_cmd`
...
任务模式一个 DAG 节点）内多次调用共用同一个容器，/workspace 里的中间产物保留，模型可以
...
接入点：main_agent 只需把 run_code() 包成一个 MainTool 即可（该包装故意不放这里，避免依赖
正被并行会话改写的 main_agent.py；见升级文档 §6 接入注记）。
"""
...
from .base import ExecuteOptions, FileWriteEntry, SandboxError, SandboxUnavailable
...
_ENTRY_JS = "/workspace/__main__.js"

# 语言 → (脚本落地路径, 执行命令)。bash 复用 run_code 的整套生命周期（并发闸/会话复用/
...
_LANG_SPEC: dict = {
    ...
    # 工作流代码节点也复用同一沙箱内核。限制 V8 堆，避免其默认堆上限突破容器的内存预算。
    "javascript": (_ENTRY_JS, f"node --max-old-space-size=256 {_ENTRY_JS}"),
}
...
# code_runner 流式解析后经 progress_callback(stage="artifact_page") 直播给前端逐页渲染。
...
# 并发闸（ADR-047 §7.4）：全局最多同时存在的 run_code 沙箱数，防狂发 run_code 打满宿主机。
...
async def _acquire_with_timeout(sem: asyncio.Semaphore, timeout: float) -> bool:
    ...
    **实测在 Python 3.11 上复现不出来**：老写法与本实现各跑 200 轮 x 4 等待者的贴线竞态，
    ...
    try:
        ...
        # 会继续排队、稍后拿到名额，而 run_code 的 try/finally 是在拿到名额之后才开始的
...
class RunCodeResult:
    ...
    # 由调用方落库（code_runner 不碰 DB）。未开 collect_workspace 时恒为空。
    ...
    def to_tool_text(self) -> str:
        ...
        if self.error:
            return f"[run_code 失败] {self.error}"
        ...
        if reviewed:
            ...
            if (self.review or {}).get("status") == "failed":
                parts.append(
                    ...
                    "用**相同文件名**重新 run_code（坏版本已存为草稿，修复通过后自动转正）。"
                    "修复完成前不要声称错误已解决。"
                )
        if self.truncated:
...
async def run_code(
    ...
    *,
    ...
    if sem is not None:
        ...
        if not acquired:
            return RunCodeResult(ok=False, error="沙箱繁忙（并发已满），请稍后重试")
    ...
    try:
        ...
        try:
            ...
            return RunCodeResult(ok=False, error=f"沙箱不可用：{e}")
        if session is not None:
        ...
        if not mk.ok and session is not None:
            ...
            mk = await sandbox.execute(_mkdir_cmd(entries), ExecuteOptions(timeout_ms=10000))
        if not mk.ok:
            return RunCodeResult(ok=False, error=f"初始化工作目录失败：{mk.stderr[:200]}")
        ...
        files_baseline = await _snapshot_workspace_files(sandbox) if collect_workspace else {}
        ...
        if collect_workspace:
            ...
            if changed_paths:
                ...
                # 产物质检改成「文件写入事件驱动」（批 3）：命中办公/文档后缀就跑既有硬校验，
                # 结果照旧以 review 状态回给调用方拼 [artifact_validity_gate=...] 标记。
                ...
                if gated and settings.SANDBOX_OUTPUT_REVIEW_ENABLED:
                    ...
                    # 主对话新统一文件系统只收集 files/，旧 visual_review 却只接在
                    # collect_outputs 分支；这会让 PPT 永远只过“能打开”而没有截图审美。
                    # 复用同一批已附着结构 review 的 entry，指定 files/ 为源目录。
                    if settings.SANDBOX_VISUAL_REVIEW_ENABLED:
                        await _progress(
                            progress_callback, "visual_reviewing", "正在渲染页面做视觉审查"
                        )
                        try:
                            from .visual_review import visual_review_outputs
                            visual = await visual_review_outputs(
                                sandbox,
                                gated_entries,
                            ...
                            logger.warning("files/ 视觉审查失败（降级交付）: %s", exc)
                            visual = None
                        if visual is not None:
                            rank = {"passed": 0, "unknown": 1, "warning": 1, "failed": 2}
                            if review_overall is None or rank.get(visual["status"], 0) > rank.get(
                                (review_overall or {}).get("status"), 0
                            ):
                                review_overall = visual
                            elif review_overall is not None and visual.get("items"):
                                review_overall = dict(review_overall)
                                review_overall["items"] = visual["items"]
                            await _progress(
                                progress_callback,
                                "visual_reviewed",
                                "已完成交付质量检查",
        ...
        if collect_outputs:
            ...
            if outputs and settings.SANDBOX_OUTPUT_REVIEW_ENABLED:
                ...
                # 第二层：渲染代表性页面 + 全文件内容提纲交独立多模态质量审查；required
                # 模式下不可用也不放行，不通过统一进入返工管线。
                if settings.SANDBOX_VISUAL_REVIEW_ENABLED:
                    await _progress(
                        progress_callback, "visual_reviewing", "正在渲染页面做视觉审查"
                    )
                    try:
                        from .visual_review import visual_review_outputs
                        v = await visual_review_outputs(
                            sandbox, outputs, task_brief=task_brief, newapi_key=newapi_key
                        )
                    except Exception as e:  # noqa: BLE001 审查失败≠产物失败，对齐上面 review_outputs()
                        # 的兜底：基础设施抖动（容器被回收/docker-py 异常等）只降级为跳过视觉审查，
                        # 不能让脚本已成功产出的 res.stdout/exit_code/outputs 被一并丢弃
                        logger.warning("视觉审查执行失败（跳过，不影响已生成产物交付）: %s", e)
                        v = None
                    if v is not None:
                        rank = {"passed": 0, "unknown": 1, "warning": 1, "failed": 2}
                        if review_overall is None or rank.get(v["status"], 0) > rank.get(
                            (review_overall or {}).get("status"), 0
                        ):
                            review_overall = v
                        await _progress(
                            progress_callback, "visual_reviewed",
                            "已完成交付质量检查",
                            {"review_complete": True},
                        )
        ...
        return RunCodeResult(ok=False, error=f"沙箱操作失败：{e}")
    finally:
...
async def _migrate_outputs(sandbox) -> tuple:  # noqa: ANN001
    ...
    `collect_outputs=False`，那里的东西既不收集也不报告。run_code 时代写成的技能、GitHub 上
