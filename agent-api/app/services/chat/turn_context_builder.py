...
def _extract_readme(payload: Any) -> str:
    """从 /ai/skill/readme 的返回里取出 SKILL.md 正文（兼容 string / {result} / {readmeContent}）。"""
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.get(
                list_url, params={"pageNo": 1, "pageSize": 1000, "enabled": 1}, headers=headers,
            )
            data = resp.json()
            ...
            for r in records:
                ...
                # SKILL.md 正文（G8）：list 记录已含则用，否则按 record id 补取 readme 端点
                readme = str(r.get("readmeContent") or r.get("readme") or "").strip()
                if not readme and record_id:
                    try:
                        rr = await client.get(readme_url, params={"id": record_id}, headers=headers)
                        readme = _extract_readme(rr.json())
                    except Exception as e:  # noqa: BLE001
                        logger.warning("技能 readme 回源失败 %s: %s", record_id, e)
                        readme = ""
                trusted.append({
                    "id": sid,
                    # record_id 供取包桥接（/ai/skill/files|file 用记录 id，非 skillId）
                    "record_id": record_id,
...
def _make_skill_packages_provider(trusted_skills: Optional[List[dict]], token: str):
    ...
    只对已校验 enabled 的可信 skill 取包（record_id 回源 /ai/skill/files|file）；无可取则返回 None。
    """
    skills = [
