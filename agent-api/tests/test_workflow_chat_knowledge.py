...
class WorkflowChatKnowledgeTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_knowledge_retrieval_sends_tenant_header_resolved_from_selected_kb(self):
        ...
        class FakeResponse:
            def json(self):
                return {"success": True, "result": {"items": [{"content": "片段", "documentName": "文档"}]}}
        ...
        with (
            patch("app.services.workflows.workflow_engine.httpx.AsyncClient", FakeAsyncClient),
            patch("app.services.workflows.workflow_engine.async_session", lambda: FakeSession(), create=True),
        ):
            quotes = await engine._retrieve_knowledge_quotes(["kb-1"], "用户问题")

        self.assertEqual(captured["headers"]["X-Access-Token"], "token-1")
        self.assertEqual(captured["headers"]["X-Tenant-Id"], "tenant-1")
        self.assertEqual(captured["json"]["knowledgeIds"], ["kb-1"])
        self.assertEqual(captured["json"]["agentId"], "app1")
        self.assertEqual(captured["json"]["scoreThreshold"], 0.4)
        self.assertNotIn("threshold", captured["json"])
        self.assertEqual(quotes, [{"q": "片段", "a": "", "sourceName": "文档", "score": None}])
    ...
    async def test_knowledge_retrieval_preserves_recalled_image_urls(self):
        ctx = RunContext(input_text="默认问题", token="token-1", user_id="u1", app_id="app1")
        engine = WorkflowEngine({"nodes": [], "edges": [], "chatConfig": {}}, ctx)
            ...
            def json(self):
                return {
                    ...
                    "result": {
                        "items": [{
                            "content": "片段",
                            "contentWithImages": "片段\n\n![知识库图片](</api/sys/common/static/knowledge/a.png>)",
                            "documentName": "文档",
                            "imageUrls": [
        ...
        engine._resolve_kb_tenant = AsyncMock(return_value=None)
        with patch("app.services.workflows.workflow_engine.httpx.AsyncClient", FakeAsyncClient):
            quotes = await engine._retrieve_knowledge_quotes(["kb-1"], "用户问题")
        ...
        self.assertEqual(quotes, [{
            ...
            "score": None,
            "text": "片段",
            "imageUrls": ["data:image/png;base64,abc", "https://example.com/image.png"],
        }])

    async def test_preview_knowledge_retrieval_does_not_use_agent_binding_scope(self):
        ctx = RunContext(input_text="默认问题", token="token-1", user_id="u1", app_id="app1", preview_only=True)
        engine = WorkflowEngine({"nodes": [], "edges": [], "chatConfig": {}}, ctx)
        captured = {}

        class FakeResponse:
            def json(self):
                return {"success": True, "result": {"items": [{"content": "草稿片段", "documentName": "文档"}]}}
        ...
        self.assertNotIn("agentId", captured["json"])
        self.assertEqual(quotes, [{"q": "草稿片段", "a": "", "sourceName": "文档", "score": None}])
